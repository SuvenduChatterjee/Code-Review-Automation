...
class StepParser(object):
    ...
    def get_step_outputs(self):
        ...
        """
        user_rules= bool(self.form['save_button']=="True")
        user_selected_violation_rules = self.form['selected_rules'].split(',')
        variables = []
        ...
        for flw_var_assignment in flw_var_assignments:            
            ...
            if not self.form['ignore_flow_output_vars_in_steps'] and \
               ...
               if (user_rules == False or (user_rules == True and "008" in user_selected_violation_rules)):
        ...
        if scriptlet is not None :
            if scriptlet.find('script').text is not None:
                ...
                if not check_try_catch_in_js(self.scriptlet):
                    if (user_rules == False or (user_rules == True and "007" in user_selected_violation_rules)):
