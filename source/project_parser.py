...
class ProjectParser():
    ...
    OO Project Parser
    """
    def __init__(self, form, isWindows):
        self.zf_ref = zipfile.ZipFile(form['jar_file'], 'r')
        self.form = form
        self.isWindows = isWindows
        self.sys_props_usage_list = []        
        self.project_name = form['jar_file'].split('-')[0].split('/' or '\\')[2]
    ...
    def set_flws_and_cfgs(self):
        ...
        for file_ref in self.zf_ref.infolist():
            if file_ref.filename.endswith('.xml'):
                ...
                elif file_ref.filename.startswith('Content/Library/'):
                    self.flows.append(file_ref)
                elif file_ref.filename.startswith('Content/Configuration/'):
                    if 'System Properties' in file_ref.filename:
                        self.sys_props.append(file_ref)
    ...
    def closeJARFile(self,closeFlag):
        if closeFlag:
            self.zf_ref.close()
            return True
        return False
